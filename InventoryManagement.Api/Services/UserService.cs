...
public class UserService : ControllerBase
{
    ...
    private readonly IUserProcessors _usersProcessors;
    private readonly IConfigProject _configProject;

    public UserService(IUserProcessors usersProcessors, IConfigProject configProject)
    {
        _usersProcessors = usersProcessors;
    ...
    public async Task<CoreResponse<string>> Login(LoginRequest login)
    {
        ...
        if (user == null || !Utility.VerifyPassword(login.Password, user.Password))
        {
            ...
            {
                Data = null,
                ResponseCode = CoreResponseCode.Unauthorized,
